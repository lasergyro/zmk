...
#define ATTR_SIZE_MOUSE COND_CODE_1(IS_ENABLED(CONFIG_ZMK_POINTING), (4), (0))
#define ATTR_SIZE_HID_INDICATORS COND_CODE_1(IS_ENABLED(CONFIG_ZMK_HID_INDICATORS), (3), (0))
#define ATTR_SIZE_GENERIC_DESKTOP 4
...
static ssize_t read_hids_generic_desktop_input_report(struct bt_conn *conn,
                                               const struct bt_gatt_attr *attr, void *buf,
                                               uint16_t len, uint16_t offset) {
    struct zmk_hid_generic_desktop_report_body *report_body = &zmk_hid_get_generic_desktop_report()->body;
    return bt_gatt_attr_read(conn, attr, buf, len, offset, report_body,
...
BT_GATT_SERVICE_DEFINE(
    ...
    BT_GATT_CHARACTERISTIC(BT_UUID_HIDS_REPORT, BT_GATT_CHRC_READ | BT_GATT_CHRC_NOTIFY,
                           BT_GATT_PERM_READ_ENCRYPT, read_hids_generic_desktop_input_report, NULL, NULL),
    BT_GATT_CCC(input_ccc_changed, BT_GATT_PERM_READ_ENCRYPT | BT_GATT_PERM_WRITE_ENCRYPT),
    BT_GATT_DESCRIPTOR(BT_UUID_HIDS_REPORT_REF, BT_GATT_PERM_READ_ENCRYPT, read_hids_report_ref,
...
int zmk_hog_send_generic_desktop_report(struct zmk_hid_generic_desktop_report_body *report) {
    ...
    if (err) {
        switch (err) {
        case -EAGAIN: {
            LOG_WRN("Generic Desktop message queue full, popping first message and queueing again");
        ...
        default:
            LOG_WRN("Failed to queue generic desktop report to send (%d)", err);
            return err;
        }
    }
